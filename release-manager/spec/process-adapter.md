...
`LegacyRestoreDescriptor.observedAuthority` and `restoreRecipe` are separate proofs covered by that one digest. `observeLegacy` matches only the immutable observed executable, argv, cwd, process identity, source and listener. `preflightLegacyRestore` and legacy `restore` use only the reviewed recipe. The recipe binds the same release SHA/path and payload executable/cwd. Payload argv may differ only by removing explicitly declared host/port flag pairs so the trusted bootstrap can inject the business listener or an isolated probe listener. The bootstrap is permission checked and non-symlink, its argv is closed, and runtime secret names/source plus typed non-secret values are explicit.
